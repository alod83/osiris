...
algorithm="knn"
speed_module="sba"
while getopts "h?i:d:a:s:" opt; do
    case "$opt" in
    h|\?)
        echo "usage run_predict.sh -a algorithm -i folder_id -d folder_date -s ske_or_sba"
        exit 0
        ;;
    i)  ID=$OPTARG
        ;;
...
#osiris_basic_dir="/data/temp_repository"
osiris_basic_dir="/data/watcher/osiris"
...
declare -a modules_name=("detection" "classification" "behaviour" "metadata" "kinematic")
...
declare -A modules[behaviour]
modules[behaviour,name]='SBA'
modules[behaviour,filepath]='SBA/behaviour_analysis.csv'
declare modules[behaviour,fields]
modules[behaviour,fields,0]='SdI'
modules[behaviour,fields,10]='Speed_Cal'
modules[behaviour,fields,11]='SH_AIS'
...
#output_file=$osiris_dir"SRP/route_prediction_${speed_module}_${algorithm}.json"
...
do
  	...
  	#sog=${features[$i,'SSA']}
  	#cog=${features[$i,'SSO']}
	sog=${features[$i,'Speed_Cal']}
  	cog=${features[$i,'SH_AIS']}
	if [ "$speed_module" == 'ske' ]; then
		sog=${features[$i,'SSA']}
  		cog=${features[$i,'SSO']}
	fi
	...
	#echo $sog
  	if [ ! -z "$lat" ] && [ ! -z "$lng" ] && [ ! -z "$sog" ] && [ ! -z "$cog" ] && [ ! -z "$bclass" ] && [ "$sog" != "null" ] && [ "$cog" != "null" ] && [ "$timestamp" != "null" ]; then
		...
		#echo $number_of_effective_SdI
		if [ "$algorithm" = "knn" ];
		then
  			python3 $module_dir/predict.py -i ${features[$i,'SdI']} -o $tmp_output_file -f -l $lat -n $lng -c $cog -s $sog -b $bclass -t "$timestamp"
		else
			python3 $module_dir/predict.py -i ${features[$i,'SdI']} -o $tmp_output_file -f -l $lat -n $lng -c $cog -s $sog -b $bclass
		fi
		echo "python3 $module_dir/predict.py -i ${features[$i,'SdI']} -o $tmp_output_file -f -l $lat -n $lng -c $cog -s $sog -b $bclass"
